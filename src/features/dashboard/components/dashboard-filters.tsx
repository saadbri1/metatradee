...
export function DashboardFiltersBar({
  ...
  onChange,
  onManageAccounts,
}: FilterControlProps & { onManageAccounts: () => void }) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-2" data-dashboard-filter-order="primary">
      <TradingFiltersControl
        symbols={symbols}
        filters={filters}
        onChange={onChange}
        count={activeCount(filters)}
      />
      <DateControl dateLabel={dateLabel} filters={filters} onChange={onChange} />
      <AccountControl
        accounts={accounts}
...
function TradingFiltersControl({
  symbols,
  filters,
  onChange,
  count,
}: Omit<FilterControlProps, 'accounts'> & { count: number }) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" className="h-10 shrink-0 bg-card px-3 font-normal shadow-sm">
            ...
            <div className="flex max-h-28 flex-wrap gap-1.5 overflow-y-auto">
              {symbols.map((symbol) => (
                <button
                  key={symbol}
                  type="button"
                  onClick={() => onChange({ ...filters, symbols: toggle(filters.symbols, symbol) })}
                  className={cn(
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="h-10 min-w-36 shrink-0 justify-between bg-card px-3 font-normal shadow-sm"
        >
...
function AccountControl({
  ...
  onChange,
  onManageAccounts,
}: Pick<FilterControlProps, 'accounts' | 'filters' | 'onChange'> & {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="h-10 min-w-40 shrink-0 justify-between bg-card px-3 font-normal shadow-sm"
        >
          <span className="flex items-center gap-2">
